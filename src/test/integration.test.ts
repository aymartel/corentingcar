...
describe('auth', () => {
  ...
  it('login con PIN incorrecto → 401', async () => {
    const r = await api('POST', '/api/auth/login', { body: { profile: 'andy', pin: '0000' } });
...
describe('solicitudes (pending → accepted) y efecto en prioridad', () => {
  it('Andy pide el día de Amigo, Amigo acepta y la prioridad efectiva cambia', async () => {
    ...
    const forbidden = await api('PATCH', `/api/requests/${id}/accept`, { token: andyToken });
    ...
    const prio = await api('GET', '/api/priority?date=2025-01-02', { token: andyToken });
    expect(data(prio).priorityUser.profile).toBe('andy');
